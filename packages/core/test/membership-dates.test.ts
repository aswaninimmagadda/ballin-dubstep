...
describe('computeEndDate', () => {
  it('1-month plan from the 1st ends on last day of month', () => {
    expect(
      computeEndDate({ startDate: '2026-06-01', durationUnit: 'months', durationValue: 1 }),
    ).toBe('2026-06-30');
  });
  it('3-month plan from mid-month', () => {
    expect(
      computeEndDate({ startDate: '2026-06-15', durationUnit: 'months', durationValue: 3 }),
    ).toBe('2026-09-14');
  });
  it('6-month plan 01-Jun → 30-Nov (matches the spec example)', () => {
    expect(
      computeEndDate({ startDate: '2026-06-01', durationUnit: 'months', durationValue: 6 }),
    ).toBe('2026-11-30');
  });
  it('12-month plan spans the year', () => {
    expect(
      computeEndDate({ startDate: '2026-01-15', durationUnit: 'months', durationValue: 12 }),
    ).toBe('2027-01-14');
  });
  it('handles Jan 31 start (end-of-month clamp)', () => {
    // 31-Jan + 1 month clamps to 28-Feb; end = 27-Feb
    expect(
      computeEndDate({ startDate: '2026-01-31', durationUnit: 'months', durationValue: 1 }),
    ).toBe('2026-02-27');
  });
  it('leap year: 1 month from 30-Jan-2028 ends 28-Feb-2028', () => {
    expect(
      computeEndDate({ startDate: '2028-01-30', durationUnit: 'months', durationValue: 1 }),
    ).toBe('2028-02-28');
  });
  it('day-based plans count the start day', () => {
    expect(
      computeEndDate({ startDate: '2026-06-01', durationUnit: 'days', durationValue: 1 }),
    ).toBe(
      '2026-06-01', // 1-day trial ends same day
    );
    expect(
      computeEndDate({ startDate: '2026-06-01', durationUnit: 'days', durationValue: 7 }),
    ).toBe('2026-06-07');
  });
