...
| @gymflow/i18n       | 7     | Telugu/English key parity (fails the build if a key is missed), template rendering. Paired with `scripts/check-i18n-coverage.mjs` in CI, which fails if a user-visible string is hard-coded in a page instead — the parity test cannot see a string that never reached a resource file. That script now proves it can fail before it is trusted to pass: it runs its own pattern over a string that must match and one that must not, and exits non-zero if either comes out wrong — because for its whole existence one escape too many had turned the pattern into something that matched no text at all, and a broken gate is indistinguishable from a clean repository                            |
