...
| Credential leak (SESSION_SECRET / MEMBER_TOKEN_SECRET) | Rotate the env var. SESSION_SECRET rotation is transparent (sessions are DB rows, the secret only signs nothing today — cookie is opaque). MEMBER_TOKEN_SECRET rotation invalidates member access tokens + QR passes instantly; members re-login via refresh tokens.                     |
...
`DATABASE_APP_URL` (host env), `SESSION_SECRET` (host env),
